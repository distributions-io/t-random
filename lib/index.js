...
*	Creates a matrix or array filled with Student's t random numbers.
*
...
function random( dims, options ) {
	...
	if ( ndims === 1 ) {
		...
		if ( dt === 'generic' ) {
			return array( len, v, rand );
		}
		return typedarray( len, dt, v, rand );
	}
	// Multidimensional data structures...
	if ( dt !== 'generic' ) {
		if ( ndims === 2 ) {
			return matrix( dims, dt, v, rand );
		}
		// TODO: dstructs-ndarray support goes here. Until then, fall through to plain arrays...
	}
	return arrayarray( dims, v, rand );
} // end FUNCTION random()
